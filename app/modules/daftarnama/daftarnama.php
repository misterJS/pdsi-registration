	...
	case 'getDataDT':
		...
		foreach ($data_table['content'] as $key => &$val) 
		{
			...
			if ($list_action['delete_data'] != 'no') {
				$val['ignore_search_action'] .= btn_link(['icon' => 'fas fa-times'
						, 'attr' => [
...
function set_data() {
	...
	$data_db['nama_orangtua'] = $_POST['nama_orangtua'];
...
function validate_form() {
	...
	$validation->setRules('nis', 'NIS', 'trim|required');
	$validation->setRules('nama_orangtua', 'Nama Orangtua', 'trim|required');
